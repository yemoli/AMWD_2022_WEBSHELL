...
def csv_file_read():
    ...
    for t in row_list:
        if t[1] == "php":
            ...
            white_list.append(t[0])
        else:
            black_list.append(t[0])
...
def features_process(negativedir, postivedir, maxfeatures):
    blackid,whiteid = csv_file_read()
    webshell_texts = read_dir(blackid)
    normal_texts = read_dir(whiteid)
    ...
    joblib.dump(countvectorizer, "model/jsp/countvectorizer_" + options.version + ".pkl")
    joblib.dump(tfidftransformer, "model/jsp/tfidftransformer_" + options.version + ".pkl")
...
def train(trainset, lables, mode, seed):
    x_train, x_test, y_train, y_test = train_test_split(trainset, lables, test_size=0.1, random_state=seed)
    clf = model_collection(mode)
    clfname = "model/jsp/" + mode + "_" + options.version + ".pkl"
...
if __name__ == "__main__":
    ...
    parser.add_option("-m", "--model", dest="mode", default="xgb", help=u'训责训练的模型种类')
